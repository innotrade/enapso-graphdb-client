...
const EnapsoGraphDBClient = {
    ...
    Endpoint: function (aOptions) {
        ...
        } else if (this.triplestore == 'ontotext-graphDB') {
        ...
        this.version = aOptions.version ? aOptions.version : 10;
...
EnapsoGraphDBClient.Endpoint.prototype = {
            ...
            this.mClient
                ...
                .then(function (aResponse) {
                    ...
                    let res = {
                        ...
                        status: aResponse.status || aResponse.statusCode,
                        message: aResponse.message || aResponse.statusMessage
                    };
                    resolve(res);
                })
                .catch(function (aError) {
    ...
    async starDogLogin(aUsername, aPassword) {
        try {
            ...
            this.mAuthorization = `bearer ${JSON.parse(lRes.body).token}`;
        } catch (err) {
            throw err;
        }
    },
    ...
    setPrefixes(prefixes) {
        ...
        const lPrefixes = {};
        if (prefixes) {
            _.each(prefixes, function (item) {
                lPrefixes[item.prefix] = item.iri;
            });
        }
        this.mClient.register(lPrefixes);
    },
