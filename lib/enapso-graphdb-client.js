...
const EnapsoGraphDBClient = {
	...
	Endpoint: function (aOptions) {
		...
		this.mQueryURL =
			aOptions.baseURL + "/repositories/" + aOptions.repository;
		this.mUpdateURL =
			aOptions.baseURL +
			"/repositories/" +
			aOptions.repository +
...
EnapsoGraphDBClient.Endpoint.prototype = {
	...
	query: async function (aQuery, options) {
		options = options || {};
		let me = this;

		let transform = options.transform ? options.transform : me.transform;

		//console.log(transform);

		return new Promise(async function (resolve, reject) {
			try {
				await me.loginStatus;
			} catch (e) {}

			if (
				me.mAuthorization &&
				me.mClient.requestDefaults &&
				me.mClient.requestDefaults.headers
			) {
				me.mClient.requestDefaults.headers.Authorization =
					me.mAuthorization;
			}
			me.mClient
				.query(aQuery)
				...
				.then(function (aBindings) {
					...
					if (transform == "toJSON") {
						resolve(me.transformBindingsToResultSet(aBindings));
					} else if (transform == "toCSV") {
						resolve(me.transformBindingsToCSV(aBindings));
					} else if (transform == "toTSV") {
						resolve(me.transformBindingsToTSV(aBindings));
					} else {
						resolve(aBindings);
					}
				})
	...
	login: async function (aUsername, aPassword) {
		let me = this;
		return (this.loginStatus = new Promise(async function (
			resolve,
			reject
		) {
			let lHeaders = {
			...
			let lOptions = {
				method: "POST",
				uri: me.mBaseURL + "/rest/login/" + aUsername,
				headers: lHeaders,
			...
			try {
				...
				me.mAuthorization = lRes.headers.authorization;
				lRes = {
					success: true,
