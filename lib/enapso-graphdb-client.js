...
EnapsoGraphDBClient.Endpoint.prototype = {
    ...
    query(aQuery, options) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (this.triplestore === 'qlever' && useQleverBindings) {
                ...
                const queryType = this.detectQueryType(aQuery);
                if (queryType === 'SELECT') {
                    this.mClient.requestDefaults.headers.Accept = 'application/x-sparqlstar-results+json, application/sparql-results+json;q=0.9, */*;q=0.8';
                } else if (queryType === 'CONSTRUCT') {
                    this.mClient.requestDefaults.headers.Accept = 'application/x-graphdb-table-results+json, application/rdf+json;q=0.9, */*;q=0.8';
                } else {
                    // Default fallback for DESCRIBE, ASK, and unknown query types
    ...
    detectQueryType(query) {
        ...
        if (normalizedQuery.includes('construct ')) return 'CONSTRUCT';
        if (normalizedQuery.includes('describe ')) return 'DESCRIBE';
        if (normalizedQuery.includes('ask ')) return 'ASK';
        return 'UNKNOWN';
    }
};
